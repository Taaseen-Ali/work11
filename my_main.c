...
struct vary_node ** second_pass() {
  ...
  for (i=0;i<lastop;i++) {
    ...
    if(op[i].opcode==VARY){
      ...
      //for every vary command calculate the step between frames
      double diff = (op[i].op.vary.end_val-op[i].op.vary.start_val)/(op[i].op.vary.end_frame-op[i].op.vary.start_frame);
      for(int k=0; k<num_frames; k++){
	curr = knobs[k];
	while(curr!=NULL || strcmp(curr->name, op[i].op.vary.p->name))
	  curr=curr->next;
	    
	if(curr==NULL){
	  curr = (struct vary_node *) calloc(1, sizeof(struct vary_node));
	  strcpy(op[i].op.vary.p->name, curr->name);
	}
	curr->value = k*diff;
      }
