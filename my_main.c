...
struct vary_node ** second_pass() {
    ...
    for(int i = 0; i < lastop; i++){
        switch(op[i].opcode){
            case VARY:
	      diff = (op[i].op.vary.end_val - op[i].op.vary.start_val) / (op[i].op.vary.end_frame - op[i].op.vary.start_frame);

	      for(int k = op[i].op.vary.start_frame; k < op[i].op.vary.end_frame; k++){
		
		curr = knobs[k];
		
		struct vary_node * to_insert = malloc(sizeof(struct vary_node));
		to_insert->next = NULL;
		strcpy(to_insert->name, op[i].op.vary.p->name);
		to_insert->value = op[i].op.vary.start_val + (k - op[i].op.vary.start_frame) * diff;
